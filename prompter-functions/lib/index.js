...
exports.PBsubmitPrompt = (0, https_1.onCall)(async (request) => {
    ...
    if (!userId || !username || !userCode) {
        throw new https_1.HttpsError("invalid-argument", "Los campos userId, username y userCode son obligatorios.");
    }
    ...
    if (userSnapshot.empty) {
        throw new https_1.HttpsError("not-found", `No se encontró ningún participante registrado con el código: ${userCode}`);
    }
    try {
        ...
        const isCredsError = error?.message?.includes("Default Credentials") ||
        ...
        throw new https_1.HttpsError("internal", `Error procesando la solicitud con IA: ${error?.message || error}`);
    }
});
/**
...
exports.PBverifyPasscode = (0, https_1.onCall)(async (request) => {
    ...
    if (attemptDoc.exists) {
        ...
        if (data && data.blockedUntil) {
            ...
            if (blockedUntil.toMillis() > now.toMillis()) {
                ...
                throw new https_1.HttpsError("resource-exhausted", `Demasiados intentos fallidos. Acceso bloqueado temporalmente por IP. Inténtalo de nuevo en ${remainingSec} segundos.`);
            }
        }
    }
    ...
    if (passcode.trim() === CONFIG_PASSCODE) {
        ...
        if (attempts >= 5) {
            ...
            blockedUntil = firestore_1.Timestamp.fromMillis(now.toMillis() + blockDurationMs);
        }
        ...
        throw new https_1.HttpsError("permission-denied", errorMsg);
    }
});
